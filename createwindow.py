...
class CreateWindow:
    def clicked_send(self, event='<Return>'):
        ...
        if len(self.username_entry.get()) < 7:
        ...
        if (self.password == self.password2):
            ...
            if (flag == False):
                ...
                self.createwindow.destroy()  # удаление окна
                 ...
                 messagebox.showinfo('Ошибка', f"Пользователь с ником {self.username} уже зарегистрирован")
        else:
    ...
    def __init__(self,parent, connection):
        ...
        self.createwindow = Toplevel(parent)
        ...
        self.main_label = Label(self.createwindow, text='Создание аккаунта', font=('Arial',15), justify=CENTER, **self.base_padding)
        # помещаем виджет в окно по принципу один виджет под другим
        self.main_label.pack()

        self.username_label = Label(self.createwindow, text='Логин', font=self.label_font, **self.base_padding)
        self.username_label.pack()

        self.username_entry = Entry(self.createwindow, bg='#fff', fg='#444', font=self.font_entry)
        self.username_entry.pack()

        self.password_label = Label(self.createwindow, text='Пароль', font=self.label_font , **self.base_padding)
        self.password_label.pack()

        self.password_entry = Entry(self.createwindow, bg='#fff', fg='#444', show='*', font=self.font_entry)
        self.password_entry.pack()

        self.password_label2 = Label(self.createwindow, text='Подтвердите пароль', font=self.label_font, **self.base_padding)
        self.password_label2.pack()

        self.password_entry2 = Entry(self.createwindow, bg='#fff', fg='#444', show='*', font=self.font_entry)
        self.password_entry2.pack()

        self.send_btn = Button(self.createwindow, text='Создать пользователя', command=self.clicked_send)
